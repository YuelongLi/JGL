...
/**
 * The algorithm class uses generic type and provides functions such as finding
 * maximum sorting. Copyright of every method in this class is subjected
 * exclusively to Yuelong Li
 * 
 * @author Yuelong Li
 * @date 2017/06/23
 */
public class Algorithms {
	...
	private static void quickSortRecursion(double[] input, int ini, int fin) {
		if (fin - ini == 0)
			return;
		int counter1 = ini;
		int counter2 = fin;
	...
	/**
	 * A special sorting algorithm that works the best when the list comprises of
	 * random yet evenly scattered digits. This method sorts things into ascending
	 * order. This method only takes in a double array for the sake of high testing
	 * speed
	 * 
	...
	/**
	 * The underlying recursion mechanism of yuelong sort, which sorts the list into
	 * ascending order.
	 * 
	 ...
	 * @param length  of the sublist: start+length = endingIndex + 1 =
	 *                firstIndexOfTheNextSublist
	 * @return the sorted list
	 */
	private static void yuelongRecursion(int keySize, int start, int length) {

		if (length <= 1)
			return;// special case1
		if (length == 2) {// special case 2
			if (orgList[start] <= orgList[start + 1])
				return;
			else {
		...
		for (int i = keySize; i < keySize + length + 1; i++) {// Accumulates the digit within the chain and shifts them
	...
	/**
	 * A very special sorting algorithm that works the best when the list comprises
	 * of random yet evenly scattered digits This method sorts things into ascending
	 * order
	 * 
	 * @param list The list to be sorted
	 * @return sorted the sorted list
	...
	/**
	 * Binary searches a sorted list (ascending order) to find the (,] interval that
	 * contains the key; returns the index of the right bound. -1 if key is outside
	 * the range of the list
	 * 
	 * @param list       a sorted list
	 * @param key        the key to be matched
	 * @param comparator 1 if the first element is larger than the second, -1 if the
	 *                   first element is smaller than second 0 if they are equal
	 * @return
	 */
	public static <T> int binarySearchRight(List<T> list, T key, Comparator<T> comparator) {
		if(comparator.compare(list.get(list.size()-1), key)>0)
			return -1;
		if(comparator.compare(list.get(0), key)<0)
			return 0;
		return getRightIndex(list, key, 0, list.size()-1, comparator);
	}

	/**
	 * Binary searches a sorted list (ascending order) to find the [,) interval that
	 * contains the key; returns the index of the left bound. -1 if key is outside
	 * the range of the list
	 * 
	 * @param list       a sorted list
	 * @param key        the key to be matched
	 ...
	 * @return
	 */
	public static <T> int binarySearchLeft(List<T> list, T key, Comparator<T> comparator) {
		if(comparator.compare(list.get(0), key)>0)
			return -1;
		if(comparator.compare(list.get(list.size()-1), key)<0)
			return list.size()-1;
		return getLeftIndex(list, key, 0, list.size()-1, comparator);
	}
	 ...
	 * @param end   inclusive
	 * @param c     comparator, 1 if the first element is larger than the second, -1
	 *              if the first element is smaller than second
	 * @return left closest index
	 */
	private static <T> int getLeftIndex(List<T> list, T key, int begin, int end, Comparator<T> c) {
//		System.out.println("begin: "+begin+", "+"end: "+end);
		if (end < begin)
			return -1;
		if (c.compare(key, list.get(end)) == 0)
			return end;
		if (end - begin <= 1 && c.compare(list.get(begin), key)<=0 && c.compare(list.get(end), key) > 0)
			return begin;
		int midPoint = (begin + end) / 2;
		if (c.compare(list.get(midPoint), key)<=0)
			return getLeftIndex(list, key, midPoint, end, c);
		else
			return getLeftIndex(list, key, begin, midPoint, c);
	}
	 ...
	 * @return left closest index
	 */
	private static <T> int getRightIndex(List<T> list, T key, int begin, int end, Comparator<T> c) {
		if (end < begin)
			return -1;
		if (c.compare(key, list.get(begin))==0)
			return end;
		if (end - begin <= 1 && c.compare(list.get(begin),key)<0 && c.compare(list.get(end), key)>=0)
			return end;
		int midPoint = (begin + end) / 2;
		if (c.compare(list.get(midPoint), key)>=0)
			return getRightIndex(list, key, begin, midPoint, c);
		else
			return getRightIndex(list, key, midPoint, end, c);
	}

	/**
	 * Finds the derivative of a function at a certain point. The copyright of the
	 * mechanism is subjected of Yuelong Li
	 * 
	 * @author Yuelong Li
